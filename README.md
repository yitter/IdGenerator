...
配置变更指是系统运行之后变更运行参数（IdGeneratorOptions选项值），请注意：

1.系统运行一段时间后，当想调整系统运行参数时，最重要的一条原则是：StartTime **只能往前**（比老值更小、距离现在更远）赋值，原因是往后赋值极大可能产生相同的时间戳。[**不推荐**在系统运行之后调整 StartTime]
