...
9007199254740992       (运行70年，达到js Number 最大值)
165399880288699493     (普通雪花算法运行1年生成的ID)
```
...
❄ ***SeqBitLength***，序列数位长，**默认值6**，取值范围 [3, 21]（建议不小于4），决定每毫秒基础生成的ID个数。如果每秒请求数不超过5W，保持默认值6即可；如果超过5W，不超过50W，建议赋值10。规则要求：WorkerIdBitLength + SeqBitLength 不超过 22。
