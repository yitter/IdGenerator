...
❄ ***WorkerId***，机器码，**最重要参数**，无默认值，必须由外部设定，默认条件下最大值63，理论最大值 2^WorkerIdBitLength-1（实际上根据实现语言不同可能会限定在 65535 或 32767，原理同 WorkerIdBitLength 的规则）。不同机器或不同应用实例不可相同，你可通过应用程序配置该值，也可通过调用外部服务获取值。针对自动注册WorkerId需求，本算法提供默认实现：通过 redis 自动注册 WorkerId 的动态库，详见“Tools\AutoRegisterWorkerId”。
